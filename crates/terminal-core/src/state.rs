...
mod tests {
    ...
    #[test]
    fn handles_8_bit_c1_osc_with_st() {
        let mut terminal = TerminalState::new(4, 10);
    ...
    #[test]
    fn renders_dec_cyrillic_charset() {
        let mut terminal = TerminalState::new(4, 25);

        terminal.append_bytes(b"\x1b(&4@ABCXYZ[\\]^_`abcxyz{|}~!\x1b(B!");

        assert_eq!(terminal.snapshot(4).lines[0], "юабцьызшэщчъЮАБЦЬЫЗШЭЩЧ␦!");
    }
    ...
    #[test]
    fn renders_iso_latin1_supplemental_charset() {
        let mut terminal = TerminalState::new(4, 20);
    ...
    #[test]
    fn renders_iso_latin_cyrillic_supplemental_charset() {
        let mut terminal = TerminalState::new(4, 20);
    ...
    #[test]
    fn renders_italian_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(Y#@[\\]`{|}~\x1b(B#");

        assert_eq!(terminal.snapshot(4).lines[0], "£§°çéùàòèì#");
    }
    ...
    #[test]
    fn renders_dutch_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(4#@[\\] {|}~\x1b(B#");

        assert_eq!(terminal.snapshot(4).lines[0], "£¾ĳ½| ¨ƒ¼´#");
    }

    #[test]
    fn renders_swedish_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(H@[\\]^`{|}~\x1b(B@");

        assert_eq!(terminal.snapshot(4).lines[0], "ÉÄÖÅÜéäöåü@");
    }

    #[test]
    fn renders_norwegian_danish_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(E@[\\]^`{|}~\x1b(B@");

        assert_eq!(terminal.snapshot(4).lines[0], "ÄÆØÅÜäæøåü@");
    }

    #[test]
    fn renders_french_canadian_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(Q#@[\\]^`{|}~\x1b(B@");

        assert_eq!(terminal.snapshot(4).lines[0], "#àâçêîôéùèû@");
    }
    ...
    #[test]
    fn renders_portuguese_nrcs_charset() {
        let mut terminal = TerminalState::new(4, 20);

        terminal.append_bytes(b"\x1b(%6#[\\] {|}~\x1b(B[");

        assert_eq!(terminal.snapshot(4).lines[0], "#ÃÇÕ ãçõ~[");
    }
