...
- Phase 016-036: TUI replay fixtures, app smoke unknown resolution, xterm/vttest compatibility expansion, representative app certification tracking, secondary device attributes, function/modified key encoding, application keypad mode, OSC 52 clipboard write/query policy, legacy/modifier-aware mouse encoding, selection drag autoscroll, OSC title updates, vttest menu replay/app runtime evidence, Tab/Esc input fixes, absolute scrollback selection, app command smoke automation, app target command smokes, representative CLI version app smokes, and DEC Special Graphics charset support are implemented.
