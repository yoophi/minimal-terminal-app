...
- Phase 158에서 `htop`이 쓰는 `CSI ?1006;1000h/l` combined mouse private mode set/reset을 parser/state test로 고정했다.
...
- `htop`: `htop 3.5.1` version smoke, runtime redraw, AppKit cell width rendering fix, background color erase fix, VT index/scroll fix, combined mouse private mode fix, quit, F10 quit, mouse F2 Setup workflow, F1 help quit, F5 tree, F2 setup, 임시 `HTOPRC` 기반 setup save와 Setup 내부 Tree view 설정 변경 smoke는 통과했다.
...
- Phase 152에서 app 내부 `htop` synthetic mouse click 기반 htop 내부 화면 전환 workflow를 자동화했다. Phase 158 이후 정확한 SGR mouse mode 처리를 기준으로 `htop-mouse-setup` target이 F2 Setup 클릭을 확인한다.
...
- Phase 153에서 임시 `HTOPRC` 경로를 사용해 app 내부 `htop` Setup에서 `Tree view` 설정을 켜고 `tree_view=1` 저장 결과를 확인하는 workflow를 자동화했다.
...
- `htop` Setup 전체 설정 항목 coverage가 필요하면 대표 항목을 추가로 분해한다.
...
- code-level로 검증된 modifier key variants 중 Up/F5 key 전체 modifier matrix, Shift/Control/Option/Shift+Option/Shift+Control/Control+Option/Shift+Control+Option F1-F12 function family, Control+Option/Shift+Control/Shift+Option/Shift+Control+Option/Shift/Control navigation family 및 Option non-word navigation family 외 나머지 navigation/function 조합을 앱별 runtime smoke로 검증한다.
...
- Phase 155에서 app 내부 `vttest` 시작 메뉴의 2번 screen features 하위 테스트 화면 진입을 자동화했다.
...
Phase 127에서 `native-control-f5-key` app target smoke를 추가했다. synthetic Control+F5 `NSEvent`가 `TerminalView keyDown:` path를 지나 xterm-style modified function key sequence `ESC [ 15 ; 5 ~`로 PTY에 전달되는 것을 shell raw readback marker `native-control-f5-key:1b5b31353b357e`로 확인했다. Phase 128에서 `native-shift-option-up-key` app target smoke를 추가해 synthetic Shift+Option+Up `NSEvent`가 modified navigation key sequence `ESC [ 1 ; 4 A`로 PTY에 전달되는 것을 marker `native-shift-option-up-key:1b5b313b3441`로 확인했다. Phase 129에서 `native-control-option-right-key` app target smoke를 추가해 synthetic Control+Option+Right `NSEvent`가 modified navigation key sequence `ESC [ 1 ; 7 C`로 PTY에 전달되는 것을 marker `native-control-option-right-key:1b5b313b3743`으로 확인했다. Phase 130에서 `native-up-modifier-matrix-key` app target smoke를 추가해 synthetic Up key `NSEvent` 7개가 Shift/Option/Control 전체 modifier 조합별 sequence `ESC [ 1 ; 2 A`부터 `ESC [ 1 ; 8 A`까지 PTY에 전달되는 것을 per-modifier marker로 확인했다. Phase 131에서 `native-f5-modifier-matrix-key` app target smoke를 추가해 synthetic F5 key `NSEvent` 7개가 Shift/Option/Control 전체 modifier 조합별 sequence `ESC [ 15 ; 2 ~`부터 `ESC [ 15 ; 8 ~`까지 PTY에 전달되는 것을 per-modifier marker로 확인했다. Phase 132에서 generic native key smoke parser와 `native-control-option-navigation-family-key` app target smoke를 추가해 Control+Option Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 133에서 `native-shift-control-navigation-family-key` app target smoke를 추가해 Shift+Control Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 135에서 `native-shift-option-navigation-family-key` app target smoke를 추가해 Shift+Option Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 136에서 `native-shift-control-option-navigation-family-key` app target smoke를 추가해 Shift+Control+Option Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 137에서 `native-shift-navigation-family-key` app target smoke를 추가해 Shift Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 138에서 `native-control-navigation-family-key` app target smoke를 추가해 Control Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 139에서 `native-option-non-word-navigation-key` app target smoke를 추가해 Option Up/Down/Home/End/PageUp/PageDown/Delete sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 141에서 `native-shift-function-f1-f4-key`와 `native-shift-function-f5-f12-key` app target smoke를 추가해 Shift F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 142에서 `native-control-function-f1-f4-key`와 `native-control-function-f5-f12-key` app target smoke를 추가해 Control F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 143에서 `native-option-function-f1-f4-key`와 `native-option-function-f5-f12-key` app target smoke를 추가해 Option F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 146에서 `native-shift-option-function-f1-f4-key`와 `native-shift-option-function-f5-f12-key` app target smoke를 추가해 Shift+Option F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 147에서 `native-shift-control-function-f1-f4-key`와 `native-shift-control-function-f5-f12-key` app target smoke를 추가해 Shift+Control F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 148에서 `native-control-option-function-f1-f4-key`와 `native-control-option-function-f5-f12-key` app target smoke를 추가해 Control+Option F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Phase 150에서 `native-shift-control-option-function-f1-f4-key`와 `native-shift-control-option-function-f5-f12-key` app target smoke를 추가해 Shift+Control+Option F1-F12 sequence가 PTY에 전달되는 것을 per-key marker로 확인했다. Option Left/Right/Backspace는 shell word navigation 정책으로 유지한다. Shift/Option/Control 전체 조합은 code-level test가 있으며, 전체 navigation/function key runtime matrix는 Full xterm Compatibility Coverage gap으로 계속 추적한다.
...
Phase 036에서 `scripts/run-app-target-smokes.sh`에 `vttest-menu` target을 추가했다. local verification environment에서 app 내부 PTY로 `vttest`를 실행하고 시작 메뉴 snapshot marker `VT100 test program`을 확인했다. Phase 151에서 `vttest-version` target을 추가해 app 내부 PTY로 `vttest -V` version output marker `VT100 test program`을 확인했다. Phase 154에서 `vttest-cursor-movement` target을 추가해 시작 메뉴에서 1번 cursor movement 하위 테스트 화면으로 진입하고 `The screen should be cleared` marker를 확인했다. Phase 155에서 `vttest-screen-features` target을 추가해 시작 메뉴에서 2번 screen features 하위 테스트 화면으로 진입하고 `Test of WRAP AROUND mode setting` marker를 확인했다. Phase 156에서 `vttest-character-sets` target을 추가해 시작 메뉴에서 3번 character sets 하위 테스트 화면으로 진입하고 `These are the installed character sets` marker를 확인했다. Phase 157에서 `vttest-double-sized` target을 추가해 시작 메뉴에서 4번 double-sized characters 하위 테스트 화면으로 진입하고 `Double-width-and-height line` marker를 확인했다. 전체 interactive menu suite 결과는 vttest Runtime Coverage gap으로 계속 추적한다.

### htop App Runtime Snapshot Smoke

상태: `partially supported`

Phase 038에서 `scripts/run-app-target-smokes.sh`에 `htop-runtime` target을 추가했다. Phase 102에서 `CSI d`와 deferred autowrap을 구현해 app 내부 PTY의 `htop` meter layout 깨짐을 수정했고, full-screen redraw snapshot marker를 `Tasks:`로 강화했다. Phase 110에서 AppKit renderer의 cell width 계산을 실제 terminal font 측정 기반으로 바꿔 visual layout 밀림을 수정했다. Phase 115에서 `htop-runtime`을 `Tasks:`, `Load average:`, `PID USER`, `Command`, `F10Quit` 다중 marker 검증으로 강화했다. Phase 119에서 배경색이 있는 trailing space를 styled snapshot에 보존하고 AppKit renderer가 span background를 cell 크기로 먼저 칠하도록 수정해 meter/header 배경 기반 layout 표시를 보강했다. Phase 125에서 erase/insert/delete 계열 blank cell이 현재 SGR 배경색을 보존하도록 수정해 `xterm-256color`의 background color erase 기대 동작과 `htop` meter/header background rendering을 보강했다. Phase 134에서 `CSI ? 7 h/l` DEC autowrap private mode를 구현하고 `htop-runtime` marker를 `Mem[` 및 `F1Help  F2Setup`까지 강화해 right-margin redraw와 function-key row layout을 보강했다. Phase 149에서 `ESC D`, `ESC E`, `ESC M`, `CSI Ps S`, `CSI Ps T` index/scroll 계열을 구현해 scroll region 기반 htop redraw를 보강했다. Phase 158에서 `CSI ?1006;1000h/l` combined mouse private mode set/reset을 parser/state level에서 고정하고 `htop-mouse-setup`으로 F2 Setup mouse click workflow를 확인한다. Phase 103에서 F5 tree toggle snapshot을 자동화했다. Phase 111에서 F2 Setup 화면 진입 snapshot을 자동화했다. Phase 121에서 임시 `HTOPRC` 기반 setup save workflow를 자동화했다. Phase 153에서 임시 `HTOPRC` 기반 Setup 내부 Tree view 설정 변경/저장 workflow를 자동화했다. Setup 전체 설정 항목 coverage는 Environment-dependent App Smoke Target gap으로 계속 추적한다.
...
Phase 044에서 `scripts/run-app-target-smokes.sh`에 `htop-f10-quit` target을 추가했다. local verification environment에서 app 내부 PTY로 `htop`을 실행하고 follow-up F10 sequence `ESC [ 21 ~` 입력 뒤 shell marker `htop-f10-ok`를 확인했다. Phase 158에서 synthetic mouse click report 뒤 F2 Setup marker `[Setup]`을 확인한다. Phase 061에서 F1 help sequence `ESC O P`와 `q q` 입력 뒤 shell marker `htop-f1-ok`를 확인했다. Phase 103에서 F5 sequence `ESC [ 15 ~` 입력 뒤 process tree marker `├─`를 확인했다. Phase 111에서 F2 sequence `ESC O Q` 입력 뒤 Setup marker `[Setup]`을 확인했다. Phase 153에서 Setup 내부 `Tree view` 설정을 켜고 임시 htoprc의 `tree_view=1` marker를 확인했다. Setup 전체 설정 항목 coverage는 Environment-dependent App Smoke Target gap으로 계속 추적한다.
