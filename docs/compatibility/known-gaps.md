...
Phase 038에서 `scripts/run-app-target-smokes.sh`에 `htop-runtime` target을 추가했다. Phase 102에서 `CSI d`와 deferred autowrap을 구현해 app 내부 PTY의 `htop` meter layout 깨짐을 수정했고, full-screen redraw snapshot marker를 `Tasks:`로 강화했다. mouse와 추가 function key workflow는 Environment-dependent App Smoke Target gap으로 계속 추적한다.
