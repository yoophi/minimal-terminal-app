...
- `htop`: `htop 3.5.1` version smoke, runtime redraw, AppKit cell width rendering fix, background color erase fix, VT index/scroll fix, quit, F10 quit, mouse open-files workflow, F1 help quit, F5 tree, F2 setup, 임시 `HTOPRC` 기반 setup save와 Setup 내부 Tree view 설정 변경 smoke는 통과했다.
...
- Phase 152에서 app 내부 `htop` synthetic mouse click 기반 process open-files workflow를 자동화했다.
...
Phase 044에서 `scripts/run-app-target-smokes.sh`에 `htop-f10-quit` target을 추가했다. local verification environment에서 app 내부 PTY로 `htop`을 실행하고 follow-up F10 sequence `ESC [ 21 ~` 입력 뒤 shell marker `htop-f10-ok`를 확인했다. Phase 152에서 synthetic mouse click report 뒤 process open-files marker `Snapshot of files open in process`를 확인했다. Phase 061에서 F1 help sequence `ESC O P`와 `q q` 입력 뒤 shell marker `htop-f1-ok`를 확인했다. Phase 103에서 F5 sequence `ESC [ 15 ~` 입력 뒤 process tree marker `├─`를 확인했다. Phase 111에서 F2 sequence `ESC O Q` 입력 뒤 Setup marker `[Setup]`을 확인했다. Phase 153에서 Setup 내부 `Tree view` 설정을 켜고 임시 htoprc의 `tree_view=1` marker를 확인했다. Setup 전체 설정 항목 coverage는 Environment-dependent App Smoke Target gap으로 계속 추적한다.
