...
| --- | --- | --- |
| `vim` | replay evidence와 app-internal edit/write/quit, mouse left press, window split key chord, PTY/grid resize redraw smoke, native AppKit window resize -> PTY size smoke가 있어 `partially supported`. | plugin redraw, Vim 내부 user-driven AppKit resize redraw workflow |
...
| `tmux` | `tmux 3.6b` version smoke, attached session workflow smoke, split-pane workflow smoke, pane resize height 비교 smoke, copy mode buffer smoke, mouse wheel smoke, nested vim edit/write/quit smoke, split-pane 내부 nested vim resize/edit/write/quit smoke, native AppKit window resize -> PTY size smoke 통과, `partially supported`. | native GUI mouse end-to-end, tmux attached client window resize/SIGWINCH workflow, 사용자 tmux 설정 workflow |
| `htop` | `htop 3.5.1` version smoke, `Tasks:`/`Load average:`/`PID USER`/`Command`/`F10Quit` marker 기반 runtime layout smoke, AppKit cell width rendering fix, styled cell background rendering fix, background color erase fix, combined mouse private mode fix, quit/F10/mouse F2 Setup/F1 help/F5 tree/F2 setup, 임시 `HTOPRC` 기반 setup save와 Setup 내부 Tree view 설정 변경 smoke가 있어 `partially supported`. | Setup 전체 설정 항목 coverage |
| `claude` / `claude-code` | `2.1.170 (Claude Code)` version smoke와 help output smoke 통과, `partially supported`. | raw input, paste, resize, spinner/redraw, Ctrl/Alt 조합, 인증/네트워크 분리 |
| `codex` / `codex-cli` | `codex-cli 0.139.0` version smoke와 help output smoke 통과, `partially supported`. | raw input, paste, resize, redraw, Ctrl/Alt 조합, 인증/네트워크 분리 |
