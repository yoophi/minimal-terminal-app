...
| `tmux` | `tmux 3.6b` version smoke, attached session workflow smoke, split-pane workflow smoke, pane resize height 비교 smoke, copy mode buffer smoke, nested vim edit/write/quit smoke 통과, `partially supported`. | mouse mode, split-pane 내부 nested TUI resize/interaction |
