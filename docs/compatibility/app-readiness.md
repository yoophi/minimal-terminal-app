...
| `htop` | `htop 3.5.1` version smoke, `Tasks:`/`Load average:`/`PID USER`/`Command`/`F10Quit` marker 기반 runtime layout smoke, AppKit cell width rendering fix, quit/F10/F1 help/F5 tree/F2 setup smoke가 있어 `partially supported`. | mouse, setup 내부 설정 변경/저장 workflow |
