...
| `htop` | `htop 3.5.1` version smoke, `Tasks:`/`Load average:`/`PID USER`/`Command`/`F10Quit` marker 기반 runtime layout smoke, AppKit cell width rendering fix, styled cell background rendering fix, background color erase fix, quit/F10/mouse open-files/F1 help/F5 tree/F2 setup, 임시 `HTOPRC` 기반 setup save smoke가 있어 `partially supported`. | setup 내부 특정 설정값 변경 workflow |
