...
| `htop` | `htop 3.5.1` version smoke, `Tasks:`/`Load average:`/`PID USER`/`Command`/`F10Quit` marker 기반 runtime layout smoke, AppKit cell width rendering fix, styled cell background rendering fix, background color erase fix, quit/F10/mouse open-files/F1 help/F5 tree/F2 setup, 임시 `HTOPRC` 기반 setup save와 Setup 내부 Tree view 설정 변경 smoke가 있어 `partially supported`. | Setup 전체 설정 항목 coverage |
