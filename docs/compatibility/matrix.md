...
| Function and modified keys | F1-F12, Shift/Option/Control modified navigation/function key variants | partially supported | `input::tests::encodes_function_keys`, `input::tests::encodes_modified_navigation_keys`, `input::tests::encodes_modified_function_keys`, `docs/compatibility/known-gaps.md` | xterm-style key encoding을 추가했다. application keypad와 모든 modifier variant는 gap으로 남긴다. |
