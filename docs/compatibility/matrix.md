...
| `fzf` | partially supported | Phase 017 environment check: `fzf 0.73.1`, `printf 'alpha\nbeta\n' &#124; fzf --filter alpha`, `scripts/run-app-target-smokes.sh` (`fzf-filter`, `fzf-interactive`, `fzf-select`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-interactive filter smoke, interactive query redraw smoke, Enter selection workflow는 통과했다. multi-select, preview, shell integration workflow는 남아 있다. |
