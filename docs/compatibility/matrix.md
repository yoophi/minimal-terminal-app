...
| `git log --oneline --graph --decorate` | partially supported | Phase 017 environment check: current repo `git log --oneline --graph --decorate -5 --no-color`, `scripts/run-app-target-smokes.sh` (`git-log`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-pager `git log --oneline -1 --no-color` smoke는 통과했다. pager interaction은 남아 있다. |
