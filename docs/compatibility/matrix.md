...
| `git log --oneline --graph --decorate` | partially supported | Phase 017 environment check: current repo `git log --oneline --graph --decorate -5 --no-color`, `scripts/run-app-target-smokes.sh` (`git-log`, `git-pager-quit`, `git-pager-page-quit`, `git-pager-search-quit`, `git-pager-horizontal-quit`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-pager output smoke, pager quit workflow, pager page navigation 뒤 quit workflow, pager search 뒤 quit workflow, pager horizontal scroll 뒤 quit workflow는 통과했다. mark 같은 추가 pager workflow는 남아 있다. |
