...
| Primary device attributes | `CSI c`, `CSI 0 c` 및 관련 응답 | supported | `parser::tests::parses_primary_device_attributes`, `state::tests::queues_primary_device_attributes_response`, `tests/compatibility.rs::primary_device_attributes_have_core_evidence` | VT100 계열 `ESC[?1;2c` 응답을 보낸다. Secondary DA는 known gap으로 남긴다. |
...
| Secondary device attributes | `CSI > c` 및 관련 응답 | not supported | `docs/compatibility/known-gaps.md` | 일부 multiplexer와 terminal-aware tool의 capability query에 필요할 수 있다. |
| Function/application keypad keys | function key, application keypad, modifier variants | not supported | `docs/compatibility/known-gaps.md` | 현재 arrow/navigation 중심이다. |
| OSC clipboard | OSC 52 clipboard | not supported | `docs/compatibility/known-gaps.md` | 보안 정책이 필요하므로 별도 gap으로 추적한다. |
...
| Selection and copy | supported | `selection::tests::*`, `state::tests::combined_snapshot_spans_scrollback_and_live_screen`, Phase 006 smoke notes | 현재 viewport가 scrollback과 live screen을 함께 포함할 수 있다. drag autoscroll은 후속 gap이다. |
...
| `htop` | not supported | Phase 017 environment check, `docs/compatibility/known-gaps.md` | local verification environment에 `htop`이 설치되어 있지 않아 runtime smoke를 수행하지 못했다. |
