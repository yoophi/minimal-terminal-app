...
| Character set designation | `ESC ( 0`, `ESC ( A`, `ESC ( B`, `ESC ) 0`, `ESC ) A`, `ESC ) B`, `ESC * 0`, `ESC * A`, `ESC * B`, `ESC + 0`, `ESC + A`, `ESC + B`, `SO`, `SI`, `ESC n`, `ESC o`, `ESC N`, `ESC O`, unsupported G-set designations | partially supported | `parser::tests::parses_ascii_charset_designation`, `parser::tests::maps_dec_special_graphics_charset`, `parser::tests::maps_g1_dec_special_graphics_with_locking_shift`, `parser::tests::maps_g2_and_g3_dec_special_graphics_with_locking_shift`, `parser::tests::maps_g2_and_g3_dec_special_graphics_with_single_shift`, `parser::tests::maps_british_nrcs_charset`, `state::tests::renders_dec_special_graphics_line_drawing`, `state::tests::renders_g1_dec_special_graphics_with_locking_shift`, `state::tests::renders_g2_and_g3_dec_special_graphics_with_locking_shift`, `state::tests::renders_g2_and_g3_dec_special_graphics_with_single_shift`, `state::tests::renders_british_nrcs_charset`, `tests/compatibility.rs::dec_special_graphics_have_core_evidence`, `docs/compatibility/known-gaps.md` | G0-G3 DEC Special Graphics line drawing은 Unicode box drawing 문자로 매핑한다. G2/G3 single shift와 British NRCS `#` -> `£` mapping도 지원한다. right-side G-set locking shift, 기타 NRCS charset은 범위 밖이다. |
...
| `htop` | partially supported | Phase 019 environment check: `/opt/homebrew/bin/htop`, `htop 3.5.1`, `scripts/run-app-target-smokes.sh` (`htop-version`, `htop-runtime`, `htop-quit`, `htop-f10-quit`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 version smoke, full-screen redraw snapshot, `q` quit workflow, F10 quit workflow는 통과했다. mouse와 추가 function key workflow는 남아 있다. |
| `fzf` | partially supported | Phase 017 environment check: `fzf 0.73.1`, `printf 'alpha\nbeta\n' &#124; fzf --filter alpha`, `scripts/run-app-target-smokes.sh` (`fzf-filter`, `fzf-interactive`, `fzf-preview`, `fzf-select`, `fzf-multi-select`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-interactive filter smoke, interactive query redraw smoke, preview workflow, Enter selection workflow, multi-select workflow는 통과했다. shell integration workflow는 남아 있다. |
