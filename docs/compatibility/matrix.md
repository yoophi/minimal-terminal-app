...
| `git log --oneline --graph --decorate` | partially supported | Phase 017 environment check: current repo `git log --oneline --graph --decorate -5 --no-color`, `scripts/run-app-target-smokes.sh` (`git-log`, `git-pager-quit`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-pager output smoke와 pager quit workflow는 통과했다. pager scroll/page navigation은 남아 있다. |
