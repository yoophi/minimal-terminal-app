...
| `tmux` | partially supported | Phase 019 environment check: `/opt/homebrew/bin/tmux`, `tmux 3.6b`, `scripts/run-app-target-smokes.sh` (`tmux-version`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 version smoke는 통과했다. pane/nested TUI workflow는 수동 smoke로 남긴다. |
