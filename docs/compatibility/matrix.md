...
| Device status report | `CSI 5n`, `CSI 6n` 및 관련 응답 | not supported | `docs/compatibility/known-gaps.md` | parser state만으로는 부족하고 PTY 응답 경로가 필요하다. |
| Mouse reporting | DEC private mouse mode와 mouse event encoding | not supported | `docs/compatibility/known-gaps.md` | 여러 editor와 multiplexer의 mouse UX에 필요하다. |
| Cursor style | `CSI Ps SP q` 및 관련 cursor style sequence | not supported | `docs/compatibility/known-gaps.md` | 현재 cursor renderer는 고정 block cursor만 사용한다. |
...
| Paste | supported | `TerminalView::paste_text_from_clipboard` app paste path | pasteboard 자동 테스트는 아직 없다. |
| Bracketed paste | supported | `terminal_core::tests::tracks_tui_modes`, app paste wrapper 구현 | editor 통합은 runtime smoke로 확인해야 한다. |
...
| `less` | partially supported | Phase 007 first compatibility expansion | `printf 'one\ntwo\nthree\n' &#124; less` |
| `vim` | partially supported | Phase 007 first compatibility expansion | `vim /tmp/minimal-terminal-smoke.txt` |
| `top` | partially supported | Phase 007 smoke target | `top` |
| `htop` | unknown | 미확인 | 로컬 설치 여부에 따라 달라진다. |
| `fzf` | unknown | 미확인 | 로컬 설치 여부에 따라 달라진다. |
| `git log --oneline --graph --decorate` | unknown | 미확인 | history가 있는 repository에서 확인해야 한다. |
