...
| `tmux` | partially supported | Phase 019 environment check: `/opt/homebrew/bin/tmux`, `tmux 3.6b`, `scripts/run-app-target-smokes.sh` (`tmux-version`, `tmux-attached-session`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 version smoke와 attached session pane input 후 shell 복원 workflow는 통과했다. pane split, resize, mouse, nested TUI workflow는 남아 있다. |
