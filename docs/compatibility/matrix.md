...
| `fzf` | partially supported | Phase 017 environment check: `fzf 0.73.1`, `printf 'alpha\nbeta\n' &#124; fzf --filter alpha`, `scripts/run-app-target-smokes.sh` (`fzf-filter`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 non-interactive filter smoke는 통과했다. interactive navigation은 남아 있다. |
