...
| `htop` | partially supported | Phase 019 environment check: `/opt/homebrew/bin/htop`, `htop 3.5.1`, `scripts/run-app-target-smokes.sh` (`htop-version`, `htop-runtime`, `htop-quit`), `docs/compatibility/known-gaps.md` | app 내부 PTY에서 version smoke, full-screen redraw snapshot, quit workflow는 통과했다. function key workflow는 수동 smoke로 남긴다. |
