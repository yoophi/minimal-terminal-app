...
- 자동 smoke: `less-follow`는 `less +F`가 append된 log line을 follow하는지 확인한다.
...
- `native-control-function-f1-f4-key`: synthetic Control+F1-F4 `NSEvent` 4개가 modified function key bytes를 쓰는지 확인
- `native-control-function-f5-f12-key`: synthetic Control+F5-F12 `NSEvent` 8개가 modified function key bytes를 쓰는지 확인
- `native-option-function-f1-f4-key`: synthetic Option+F1-F4 `NSEvent` 4개가 modified function key bytes를 쓰는지 확인
...
- `native-control-option-function-f1-f4-key`: synthetic Control+Option+F1-F4 `NSEvent` 4개가 modified function key bytes를 쓰는지 확인
- `native-control-option-function-f5-f12-key`: synthetic Control+Option+F5-F12 `NSEvent` 8개가 modified function key bytes를 쓰는지 확인
- `native-shift-control-option-function-f1-f4-key`: synthetic Shift+Control+Option+F1-F4 `NSEvent` 4개가 modified function key bytes를 쓰는지 확인
- `native-shift-control-option-function-f5-f12-key`: synthetic Shift+Control+Option+F5-F12 `NSEvent` 8개가 modified function key bytes를 쓰는지 확인
...
- `native-shift-control-option-navigation-family-key`: synthetic Shift+Control+Option navigation key `NSEvent` 9개가 Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete modified navigation key bytes를 쓰는지 확인
- `native-shift-navigation-family-key`: synthetic Shift navigation key `NSEvent` 9개가 Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete modified navigation key bytes를 쓰는지 확인
- `native-control-navigation-family-key`: synthetic Control navigation key `NSEvent` 9개가 Up/Down/Right/Left/Home/End/PageUp/PageDown/Delete modified navigation key bytes를 쓰는지 확인
- `native-option-non-word-navigation-key`: synthetic Option navigation key 중 shell word navigation 정책과 충돌하지 않는 Up/Down/Home/End/PageUp/PageDown/Delete 7개가 modified navigation key bytes를 쓰는지 확인
...
- `htop-setup-toggle-tree`: 임시 `HTOPRC` 경로로 `htop` 실행 후 F2 Setup 내부 `Tree view` 항목을 켜고 저장해 `tree_view=1` 또는 `.tree_view=1` 확인
...
- `vttest-version`: `vttest -V` version output snapshot
- `vttest-menu`: `vttest` 시작 메뉴 snapshot
- `vttest-cursor-movement`: `vttest` 시작 메뉴에서 1번 cursor movement 하위 테스트에 진입하고 안내 marker 확인
- `vttest-screen-features`: `vttest` 시작 메뉴에서 2번 screen features 하위 테스트에 진입하고 wrap-around mode 안내 marker 확인
- `vttest-character-sets`: `vttest` 시작 메뉴에서 3번 character sets 하위 테스트에 진입하고 character set 안내 marker 확인
- `vttest-double-sized`: `vttest` 시작 메뉴에서 4번 double-sized characters 하위 테스트에 진입하고 double-width/double-height 안내 marker 확인
- `vttest-dsr`: `vttest` terminal reports 메뉴에서 DSR 5/6 응답 marker 확인
- `vttest-primary-da`: `vttest` terminal reports 메뉴에서 primary DA 응답 marker 확인
- `vttest-secondary-da`: `vttest` terminal reports 메뉴에서 secondary DA 응답 marker 확인
...
- native GUI event에서 시작되는 mouse interaction workflow
...
- 자동 smoke: `vim-window-split`은 clean `vim`에서 `<C-W>s` key chord 후 `winnr('$') == 2`를 확인한다.
